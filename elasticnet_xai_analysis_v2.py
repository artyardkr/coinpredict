...
ax1.set_title(f'Train vs Test R² (Gap: {r2_train-r2_test:.3f})', fontsize=12, fontweight='bold')
ax1.grid(True, alpha=0.3, axis='y')
ax1.text(0.5, 0.5, overfit_status, ha='center', va='center',
        transform=ax1.transAxes, fontsize=10, fontweight='bold',
        bbox=dict(boxstyle='round', facecolor='yellow' if '⚠️' in overfit_status else 'lightgreen', alpha=0.7))
...
ax2.set_xlabel('학습 데이터 크기', fontweight='bold')
ax2.set_ylabel('R² Score', fontweight='bold')
ax2.set_title('Learning Curve (과적합 검사)', fontsize=12, fontweight='bold')
ax2.legend()
...
ax3.set_title(f'Train Residuals (평균: ${residuals_train.mean():.0f})', fontsize=12, fontweight='bold')
...
ax4.set_title(f'Test Residuals (평균: ${residuals_test.mean():.0f})', fontsize=12, fontweight='bold')
...
ax5.barh(range(len(top_coef)), top_coef['Coefficient'], color=colors, alpha=0.7)
ax5.set_yticks(range(len(top_coef)))
ax5.set_yticklabels(top_coef['Feature'], fontsize=7)
ax5.set_xlabel('계수', fontweight='bold')
ax5.set_title('Top 20 변수 계수 (V2)', fontsize=12, fontweight='bold')
ax5.axvline(0, color='black', linewidth=1)
ax5.grid(True, alpha=0.3, axis='x')

# (6) SHAP Importance Top 20
...
ax6.set_yticklabels(top_shap['Feature'], fontsize=7)
ax6.set_xlabel('평균 |SHAP Value|', fontweight='bold')
ax6.set_title('Top 20 SHAP Importance (V2)', fontsize=12, fontweight='bold')
ax6.grid(True, alpha=0.3, axis='x')

# (7) 누적 중요도
ax7 = plt.subplot(5, 4, 7)
ax7.plot(range(1, len(importance_df)+1), importance_df['Cumulative'], linewidth=2, color='purple')
ax7.axhline(0.8, color='red', linestyle='--', linewidth=1, label='80%')
ax7.axvline(n_features_80, color='red', linestyle='--', linewidth=1)
ax7.set_xlabel('변수 개수', fontweight='bold')
ax7.set_ylabel('누적 중요도', fontweight='bold')
ax7.set_title('누적 Feature Importance', fontsize=12, fontweight='bold')
ax7.legend()
ax7.grid(True, alpha=0.3)
ax7.text(n_features_80, 0.85, f'{n_features_80}개', ha='center', fontweight='bold')

# (8) 계수 분포
ax8 = plt.subplot(5, 4, 8)
ax8.hist(coefficients[coefficients != 0], bins=30, color='steelblue', alpha=0.7, edgecolor='black')
ax8.axvline(0, color='red', linestyle='--', linewidth=2)
ax8.set_xlabel('계수 값', fontweight='bold')
ax8.set_ylabel('빈도', fontweight='bold')
ax8.set_title('계수 분포 (Non-zero)', fontsize=12, fontweight='bold')
ax8.grid(True, alpha=0.3, axis='y')

# (9) SHAP Summary Plot
ax9 = plt.subplot(5, 4, (9, 10))
shap.summary_plot(shap_values_test, X_test_scaled, feature_names=feature_cols,
                  show=False, max_display=20, plot_size=(8, 6))
plt.title('SHAP Summary Plot (Top 20, V2)', fontsize=12, fontweight='bold')
...
ax11.set_title('계수 부호 분포 (V2)', fontsize=12, fontweight='bold')
ax11.grid(True, alpha=0.3, axis='y')

# (12) 신규 V2 변수 중요도
ax12 = plt.subplot(5, 4, 13)
new_vars_top = new_vars_shap.head(10)
if len(new_vars_top) > 0:
    ax12.barh(range(len(new_vars_top)), new_vars_top['SHAP_Importance'], color='orange', alpha=0.7)
    ax12.set_yticks(range(len(new_vars_top)))
    ax12.set_yticklabels(new_vars_top['Feature'], fontsize=7)
    ax12.set_xlabel('SHAP 중요도', fontweight='bold')
    ax12.set_title('🆕 신규 V2 변수 Top 10', fontsize=12, fontweight='bold')
    ax12.grid(True, alpha=0.3, axis='x')

# (13) Residual vs Fitted (Train)
ax13 = plt.subplot(5, 4, 14)
ax13.scatter(y_pred_train, residuals_train, alpha=0.3, s=10, color='steelblue')
ax13.axhline(0, color='red', linestyle='--', linewidth=2)
ax13.set_xlabel('예측값 ($)', fontweight='bold')
ax13.set_ylabel('잔차 ($)', fontweight='bold')
ax13.set_title('Residual vs Fitted (Train)', fontsize=12, fontweight='bold')
ax13.grid(True, alpha=0.3)

# (14) Residual vs Fitted (Test)
ax14 = plt.subplot(5, 4, 15)
ax14.scatter(y_pred_test, residuals_test, alpha=0.3, s=10, color='coral')
ax14.axhline(0, color='red', linestyle='--', linewidth=2)
ax14.set_xlabel('예측값 ($)', fontweight='bold')
ax14.set_ylabel('잔차 ($)', fontweight='bold')
ax14.set_title('Residual vs Fitted (Test)', fontsize=12, fontweight='bold')
ax14.grid(True, alpha=0.3)

# (15) 요약
ax15 = plt.subplot(5, 4, (16, 20))
ax15.axis('off')
summary = f"""
【ElasticNet V2 XAI 분석 요약】

1. 모델 성능:
   Train R²: {r2_train:.4f} | RMSE: ${rmse_train:,.0f}
   Test R²:  {r2_test:.4f} | RMSE: ${rmse_test:,.0f}
   Gap: {r2_train - r2_test:.4f} → {overfit_status}

2. 변수 선택 (V2):
   총 변수: {len(feature_cols)}개 (V1: 88개 → V2: {len(feature_cols)}개)
   사용 변수: {np.sum(coefficients != 0)}개
   제거 변수: {np.sum(coefficients == 0)}개 ({np.sum(coefficients == 0)/len(coefficients)*100:.1f}%)

3. 신규 V2 변수:
   추가: {len(new_vars_found)}개
   사용: {new_vars_used}개 ({new_vars_used/len(new_vars_found)*100:.1f}%)
   Top 신규: {new_vars_shap.iloc[0]['Feature'] if len(new_vars_shap) > 0 else 'N/A'}

4. Top 5 중요 변수:
   {coef_df_nonzero.iloc[0]['Feature'][:30]}: {coef_df_nonzero.iloc[0]['Coefficient']:+.4f}
   {coef_df_nonzero.iloc[1]['Feature'][:30]}: {coef_df_nonzero.iloc[1]['Coefficient']:+.4f}
   {coef_df_nonzero.iloc[2]['Feature'][:30]}: {coef_df_nonzero.iloc[2]['Coefficient']:+.4f}
   {coef_df_nonzero.iloc[3]['Feature'][:30]}: {coef_df_nonzero.iloc[3]['Coefficient']:+.4f}
   {coef_df_nonzero.iloc[4]['Feature'][:30]}: {coef_df_nonzero.iloc[4]['Coefficient']:+.4f}

5. 80% 설명력:
   필요 변수: {n_features_80}개 ({n_features_80/len(feature_cols)*100:.1f}%)

6. 과적합 검증:
   Learning Curve Gap: {train_scores_mean[-1] - test_scores_mean[-1]:.3f}
   Residual 자기상관: {residual_autocorr_test:.3f}
   {'✅ 모델 건강' if overfit_score < 0.15 and abs(residual_autocorr_test) < 0.2 else '⚠️ 개선 필요'}

7. 계수 vs SHAP:
   상관계수: {corr:.3f}
   {'→ 일치도 높음' if corr > 0.8 else '→ 일치도 보통' if corr > 0.5 else '→ 일치도 낮음'}
"""
ax15.text(0.05, 0.95, summary, fontsize=9, verticalalignment='top',
         family='monospace', transform=ax15.transAxes)
ax15.set_title('종합 요약 (V2)', fontsize=14, fontweight='bold', loc='left')

plt.tight_layout()
plt.savefig('elasticnet_xai_analysis_v2.png', dpi=300, bbox_inches='tight')
print("✅ 저장: elasticnet_xai_analysis_v2.png")
