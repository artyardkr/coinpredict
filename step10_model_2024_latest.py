...
print("=" * 70)
print("Phase 3: 모델 훈련 (2024년~최신 데이터, 7:3 분할)")
print("=" * 70)
...
# 2024년 1월 1일 이후 데이터만 필터링
df_2024 = df[df.index >= '2024-01-01'].copy()
print(f"2024년 이후 데이터: {df_2024.shape} ({df_2024.index[0].date()} ~ {df_2024.index[-1].date()})")

# 타겟 설정
df_2024['target'] = df_2024['Close'].shift(-1)
df_2024 = df_2024.dropna(subset=['target'])

print(f"타겟 생성 후: {df_2024.shape}")
...
# BTC 가격 통계
print(f"\nBTC 가격 통계 (2024년 이후):")
print(f"  최소: ${df_2024['Close'].min():,.2f}")
print(f"  최대: ${df_2024['Close'].max():,.2f}")
print(f"  평균: ${df_2024['Close'].mean():,.2f}")
print(f"  표준편차: ${df_2024['Close'].std():,.2f}")
...
# 가격 범위
print(f"\n훈련 기간 가격: ${y_train.min():,.2f} ~ ${y_train.max():,.2f}")
print(f"테스트 기간 가격: ${y_test.min():,.2f} ~ ${y_test.max():,.2f}")
...
# ===== 5. 모델별, 특성 집합별 훈련 =====
print("\n" + "=" * 70)
print("4. 모델 훈련 및 평가 (2024년 이후 데이터)")
print("=" * 70)
...
results_df.to_csv('model_results_2024.csv', index=False)
print("✓ model_results_2024.csv")

print("\n최고 성능 모델 (Test R² 기준):")
print("-" * 70)
for i, row in results_df.head(5).iterrows():
    print(f"{i+1}. {row['model']:20} | R²: {row['test_r2']:7.4f} | RMSE: ${row['test_rmse']:10,.2f} | MAPE: {row['test_mape']:5.2f}% | {int(row['n_features'])}개")
...
axes[0, 0].set_title('R² Score vs Number of Features (2024+ Data)', fontsize=12, fontweight='bold')
axes[0, 0].legend()
...
axes[1, 0].plot([results_df['train_r2'].min(), results_df['train_r2'].max()],
...
plt.tight_layout()
plt.savefig('model_performance_2024.png', dpi=300, bbox_inches='tight')
print("✓ model_performance_2024.png")
...
print(f"\n2021년 최고 성능 ({best_2021['model']}):")
print(f"  Test R²: {best_2021['test_r2']:.4f}")
print(f"  Test RMSE: ${best_2021['test_rmse']:,.2f}")
print(f"  Test MAPE: {best_2021['test_mape']:.2f}%")

print(f"\n2024년 최고 성능 ({best_model['model']}):")
print(f"  Test R²: {best_model['test_r2']:.4f}")
print(f"  Test RMSE: ${best_model['test_rmse']:,.2f}")
print(f"  Test MAPE: {best_model['test_mape']:.2f}%")

print(f"\n성능 변화:")
r2_change = best_model['test_r2'] - best_2021['test_r2']
rmse_change_pct = ((best_model['test_rmse'] - best_2021['test_rmse']) / best_2021['test_rmse']) * 100
mape_change = best_model['test_mape'] - best_2021['test_mape']

print(f"  R² 변화: {r2_change:+.4f}")
print(f"  RMSE 변화: {rmse_change_pct:+.1f}%")
print(f"  MAPE 변화: {mape_change:+.2f}%p")

print("\n" + "=" * 70)
print("2024년 이후 데이터 모델 훈련 완료!")
print("=" * 70)
