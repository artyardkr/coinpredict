#!/usr/bin/env python3
"""
Step 28: ETF 이후 기간 ElasticNet 모델 (2024.01.10 ~ 2025.10.14)
...
ETF 이후 특징:
- ETF 변수 포함 (IBIT, FBTC, GBTC, ARKB, BITB)
- 기관 수요 반영
- 시장 구조 변화 포착
"""
...
# ========================================
# 1. Load Data & ETF 이후 필터링
# ========================================
print("=" * 80)
print("ETF 이후 기간 ElasticNet 모델 (2024.01.10 ~ 2025.10.14)")
print("=" * 80)
...
# ETF 이후만 필터링 (2024년 1월 10일 이후)
ETF_DATE = '2024-01-10'
df_post = df[df['Date'] >= ETF_DATE].copy()

print(f"\n전체 데이터: {len(df)} samples ({df['Date'].min()} ~ {df['Date'].max()})")
print(f"ETF 이후 데이터: {len(df_post)} samples ({df_post['Date'].min()} ~ {df_post['Date'].max()})")
print(f"ETF 이후 기간: {(df_post['Date'].max() - df_post['Date'].min()).days} days")

# Create target: next day Close
df_post['target'] = df_post['Close'].shift(-1)
df_post = df_post[:-1].copy()

print(f"타겟 생성 후: {len(df_post)} samples")
...
ema_sma_cols = [col for col in df_post.columns if ('EMA' in col or 'SMA' in col) and 'close' in col.lower()]
exclude_cols.extend(ema_sma_cols)

# Bollinger Bands (data leakage)
bb_cols = [col for col in df_post.columns if col.startswith('BB_')]
exclude_cols.extend(bb_cols)

exclude_cols = list(set(exclude_cols))

feature_cols = [col for col in df_post.columns if col not in exclude_cols]

# ETF 변수 확인
etf_vars = [col for col in feature_cols if any(x in col for x in ['IBIT', 'FBTC', 'ARKB', 'BITB', 'GBTC', 'Total_BTC_ETF'])]

print(f"전체 컬럼: {len(df_post.columns)}")
print(f"제외 컬럼: {len(exclude_cols)}")
print(f"사용 변수: {len(feature_cols)}")
print(f"\n포함된 ETF 변수 ({len(etf_vars)}개):")
for var in etf_vars:
    print(f"  - {var}")

# Handle inf and NaN
for col in feature_cols:
    df_post[col] = df_post[col].replace([np.inf, -np.inf], np.nan)
    df_post[col] = df_post[col].fillna(method='ffill').fillna(method='bfill')
...
split_date = df_post['Date'].iloc[split_idx]

train_mask = df_post['Date'] < split_date
test_mask = df_post['Date'] >= split_date

train_df = df_post[train_mask].copy()
test_df = df_post[test_mask].copy()
...
# ETF 변수가 선택되었는지 확인
lasso_etf_vars = [v for v in lasso_selected_vars if any(x in v for x in ['IBIT', 'FBTC', 'ARKB', 'BITB', 'GBTC', 'Total_BTC_ETF'])]
print(f"Lasso 선택된 ETF 변수 ({len(lasso_etf_vars)}개): {lasso_etf_vars}")
...
# ETF 변수가 선택되었는지 확인
ridge_etf_vars = [v for v in ridge_selected_vars if any(x in v for x in ['IBIT', 'FBTC', 'ARKB', 'BITB', 'GBTC', 'Total_BTC_ETF'])]
print(f"Ridge 선택된 ETF 변수 ({len(ridge_etf_vars)}개): {ridge_etf_vars}")
...
# ETF 변수 최종 확인
union_etf_vars = [v for v in selected_vars_union if any(x in v for x in ['IBIT', 'FBTC', 'ARKB', 'BITB', 'GBTC', 'Total_BTC_ETF'])]
print(f"\n최종 선택된 ETF 변수 ({len(union_etf_vars)}개):")
for var in union_etf_vars:
    print(f"  ✅ {var}")

# Lasso에만 있는 변수
lasso_only = set(lasso_selected_vars) - set(ridge_selected_vars)
print(f"\nLasso에만:  {len(lasso_only)}개")
...
for i, (var, coef) in enumerate(top20_vars, 1):
    etf_mark = "🔥" if any(x in var for x in ['IBIT', 'FBTC', 'ARKB', 'BITB', 'GBTC', 'Total_BTC_ETF']) else "  "
    print(f"  {i:2d}. {etf_mark} {var:30s} {coef:10.4f}")
...
# ETF 변수 최종 사용 확인
en_etf_vars = [v for v in en_nonzero_vars if any(x in v for x in ['IBIT', 'FBTC', 'ARKB', 'BITB', 'GBTC', 'Total_BTC_ETF'])]
print(f"ElasticNet 최종 사용 ETF 변수 ({len(en_etf_vars)}개): {en_etf_vars}")
...
X_full = df_post[selected_vars_union].values
y_full = df_post['target'].values
...
# Walk-Forward parameters (ETF 이후는 데이터가 적으므로 window 축소)
window_size = 126  # 6개월 (ETF 이후 데이터 적음)
test_size = 30     # 30일

wf_results = []
wf_dates = []

for i in range(window_size, len(df_post) - test_size, test_size):
    # Train window
    ...
    # Get data
    X_wf_train = df_post[selected_vars_union].iloc[train_start:train_end].values
    y_wf_train = df_post['target'].iloc[train_start:train_end].values
    X_wf_test = df_post[selected_vars_union].iloc[test_start:test_end].values
    y_wf_test = df_post['target'].iloc[test_start:test_end].values
    ...
    wf_dates.append(df_post['Date'].iloc[test_start])

wf_mean = np.mean(wf_results)
wf_std = np.std(wf_results)

print(f"Walk-Forward 설정:")
print(f"  Window: {window_size}일 (6개월)")
...
selected_vars_df['Is_ETF_Var'] = selected_vars_df['Variable'].apply(lambda x: any(etf in x for etf in ['IBIT', 'FBTC', 'ARKB', 'BITB', 'GBTC', 'Total_BTC_ETF']))
selected_vars_df = selected_vars_df.sort_values('Abs_ElasticNet_Coef', ascending=False)
selected_vars_df.to_csv('etf_post_selected_variables.csv', index=False)
print("Saved: etf_post_selected_variables.csv")
...
performance_df.to_csv('etf_post_model_performance.csv', index=False)
print("Saved: etf_post_model_performance.csv")

# Backtesting results
backtest_df.to_csv('etf_post_backtesting_results.csv', index=False)
print("Saved: etf_post_backtesting_results.csv")
...
validation_df.to_csv('etf_post_validation_results.csv', index=False)
print("Saved: etf_post_validation_results.csv")
...
ax1.set_title('Model Performance (ETF 이후)', fontweight='bold')
ax1.set_xticks(x)
...
vars_top15 = [v[0][:20] for v in top15]
coefs_top15 = [v[1] for v in top15]
colors_top15 = ['#f39c12' if any(x in v[0] for x in ['IBIT', 'FBTC', 'ARKB', 'BITB', 'GBTC', 'Total_BTC_ETF']) else '#2ecc71' if v[1] > 0 else '#e74c3c' for v in top15]
...
ax3.set_title('ElasticNet 계수 TOP 15 (🟠=ETF)', fontweight='bold')
...
if len(wf_results) > 0:
    ...
    ax6.text(0.5, 0.5, 'Not enough data\nfor Walk-Forward', ha='center', va='center', fontsize=12)
    ax6.set_title('Walk-Forward Validation', fontweight='bold')
...
ax7.set_title('백테스팅 결과 (ETF 이후)', fontweight='bold')
...
plt.savefig('etf_post_elasticnet_analysis.png', dpi=300, bbox_inches='tight')
print("Saved: etf_post_elasticnet_analysis.png")

# ========================================
# 14. Final Summary
# ========================================
print("\n" + "=" * 80)
print("최종 요약 (ETF 이후 기간)")
print("=" * 80)

best_strategy = backtest_df.loc[backtest_df['Sharpe Ratio'].idxmax()]

print(f"""
📊 ETF 이후 ElasticNet 모델 분석 결과

🗓️ 데이터 기간:
   - 전체: {df_post['Date'].min().date()} ~ {df_post['Date'].max().date()} ({len(df_post)} days)
...
🔧 변수 선택:
   - 원본 변수: {len(feature_cols)}개 (ETF 변수 포함!)
   ...
   - **ETF 변수 사용**: {len(en_etf_vars)}개 {en_etf_vars}
...
🏆 결론:
   ...
   {'✅ ETF 변수가 모델에 기여!' if len(en_etf_vars) > 0 else '⚠️ ETF 변수 미사용'}
""")

print("=" * 80)
print("Step 28 (ETF 이후) Completed!")
print("=" * 80)
